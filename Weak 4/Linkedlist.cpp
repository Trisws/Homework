...
void removeBefore(List *&L, int val)
{
    ...
    {
        return; // No node before the head or list is empty
    }
    NODE *current = L->p_head;
    NODE *prev = nullptr;
    while (current->p_next && current->p_next->key != val )
    {
        prev = current;
        current = current->p_next;
    }
    ...
    if (current == L->p_head)
    {
        L->p_head = current->p_next; // Remove head
    }
    else
    {
        prev->p_next = current->p_next; // Remove node before the value
        delete current;
    }
    return;
}
...
void removeAfter(List *&L, int val)
{
    ...
    if (current == nullptr || current == L->p_tail)
    {
...
bool addPos(List *&L, int data, int pos)
{
    ...
    if (pos == 0)
    {
        ...
        if (L->p_tail == nullptr)
        {
            L->p_tail = newNode; // If the list was empty
        }
        return true;
    }
    NODE *current = L->p_head;
...
bool addBefore(List *&L, int data, int val)
{
    ...
    {
        addHead(L, data);
        return true; // Insert before head
    }
    NODE *current = L->p_head;
    NODE *prev = nullptr;
    while (current && current->key != val)

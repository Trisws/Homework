...
void removeTail(List *&L)
{
    ...
    {
        L->p_head = nullptr; // List is now empty
    }
    delete temp;
    return;
}
...
void removeAll(List *&L)
{
    ...
    L->p_tail = nullptr;
    delete L;
    L = nullptr; // Set the pointer to null
    return;
}
...
void removeBefore(List *&L, int val)
{
    ...
    {
        return; // List is empty or no node before the given value
    }
    NODE *current = L->p_head;
    ...
    if (current == nullptr || current == L->p_head)
    {
        return; // Value not found or no node before the value
    }
   NODE* NodeToRemove = current->p_prev;
    if (NodeToRemove == L->p_head)
    {
        L->p_head = current; // Update head
    }
    else
    {
        NodeToRemove->p_prev->p_next = current; // Remove node before the value
    }
    current->p_prev = NodeToRemove->p_prev; // Update the previous pointer of the current node
    delete NodeToRemove;
    return;
}
...
void removeAfter(List *&L, int val)
{
    ...
    {
        return; // Value not found or no node after the value
    }
    NODE *NodeToRemove = current->p_next;
    current->p_next = NodeToRemove->p_next; // Remove node after the value
    if (NodeToRemove->p_next != nullptr)
    {
        NodeToRemove->p_next->p_prev = current; // Update the previous pointer of the next node
    }
    else
    {
        L->p_tail = current; // Update tail if necessary
    }
    delete NodeToRemove;
    return;
}
...
bool addPos(List *&L, int data, int pos)
{
    ...
    {
        current->p_next->p_prev = newNode; // Update the previous pointer of the next node
    }
    else
    {
        L->p_tail = newNode; // Update tail if necessary
    }
    current->p_next = newNode; // Insert the new node
    newNode->p_prev = current; // Update the previous pointer of the new node
    return true;
}
...
void removePos(List *&L, int data, int pos)
{
    ...
    if (current == L->p_tail)
    {
        L->p_tail = prev; // Update tail if necessary
        delete current;
        prev->p_next = nullptr;
    }
    else
    {
        prev->p_next = current->p_next; // Remove node at position
        if (current->p_next != nullptr)
        {
            current->p_next->p_prev = prev; // Update the previous pointer of the next node
        }
        delete current;
    }
    return;
...
bool addBefore(List *&L, int data, int val)
{
    ...
    {
        current->p_prev->p_next = newNode; // Update the next pointer of the previous node
    }
    else
    {
        L->p_head = newNode; // Update head if necessary
    }
    current->p_prev = newNode; // Update the previous pointer of the current node
    return true;
}
...
List *reverseList(List *L)
{
    ...
    while (current)
    {
        next = current->p_next;
        current->p_next = prev;
        current->p_prev = next; // Update the previous pointer
        prev = current;
        current = next;
    }
    L->p_tail = L->p_head; // Update tail
    L->p_head = prev;      // Update head
    return L;
}
